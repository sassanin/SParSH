# ===================================================================
# This is FindUMFPACK.cmake file for the SParSH Version 1.0
...
if(NOT UMFPACK_FOUND)
 if(USE_SYSTEM_UMFPACK)   
  ...
  if(NOT UMFPACK_LIBRARY)
    message("UMFPACK not found in the system, so checking the availability in SParSH for the selected SParSH_ARCH=${SParSH_ARCH}")
    find_path(UMFPACK_INCLUDE_DIR  umfpack.h PATHS ${SParSH_EXTLIB_PATH}/UMFPACK/Include)
    find_library(UMFPACK_LIBRARY NAMES umfpack_${SParSH_ARCH} PATHS ${SParSH_EXTLIB_PATH}/UMFPACK/Lib) 
    get_filename_component(UMFPACK_LIBDIR ${UMFPACK_LIBRARY} PATH)
    find_library(UMFPACK_LIBRARY_SUITESE NAMES suitesparseconfig_${SParSH_ARCH} PATHS ${UMFPACK_LIBDIR})
    find_library(UMFPACK_LIBRARY_AMD NAMES amd_${SParSH_ARCH} PATHS ${UMFPACK_LIBDIR})
    find_library(UMFPACK_LIBRARY_CHOLMOD NAMES cholmod_${SParSH_ARCH} PATHS ${UMFPACK_LIBDIR}) 
    find_library(UMFPACK_LIBRARY_CCOLMOD NAMES ccolamd_${SParSH_ARCH} PATHS ${UMFPACK_LIBDIR})   
    find_library(UMFPACK_LIBRARY_CAMD NAMES camd_${SParSH_ARCH} PATHS ${UMFPACK_LIBDIR})    
    find_library(UMFPACK_LIBRARY_COLAMD NAMES colamd_${SParSH_ARCH} PATHS ${UMFPACK_LIBDIR})    
  endif(NOT UMFPACK_LIBRARY)
  
  if(UMFPACK_LIBRARY)  
    # deps for mumps
    find_library(A_LIB NAMES scalapack PATHS ${UMFPACK_LIBDIR} ${CMAKE_LIBRARY_PATH})
    find_library(PARMETIS_LIB NAMES parmetis PATHS ${UMFPACK_LIBDIR} ${CMAKE_LIBRARY_PATH} ${SParSH_EXTLIB_PATH}/Metis)
    find_library(METIS_LIB NAMES metis PATHS ${UMFPACK_LIBDIR} ${CMAKE_LIBRARY_PATH})     
    if(NOT SCALAPACK_LIB)
       find_library(SCALAPACK_LIB NAMES scalapack_${SParSH_ARCH} PATHS ${SParSH_EXTLIB_PATH}/MPIBLACS)
    endif()
     if(NOT PARMETIS_LIB)
         find_library(PARMETIS_LIB NAMES parmetis_${SParSH_ARCH} PATHS ${SParSH_EXTLIB_PATH}/Metis)
    endif()
    if(NOT METIS_LIB)
      find_library(METIS_LIB NAMES metis_${SParSH_ARCH} PATHS ${SParSH_EXTLIB_PATH}/Metis)   
    endif()      
